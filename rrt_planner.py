...
class RRTPlanner(object):
    ...
    def steer_towards(self, s_nearest, s_rand, max_radius):
        ...
        if s_rand.euclidean_distance(s_nearest) <= max_radius:
            ...
            x = s_nearest.x + int(max_radius * ((s_rand.x - s_nearest.x) / s_rand.euclidean_distance(s_nearest)))
            y = s_nearest.y + int(max_radius * ((s_rand.y - s_nearest.y) / s_rand.euclidean_distance(s_nearest)))
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        for i in range(max_checks):
            d = float(i) / max_checks * s_from.euclidean_distance(s_to)
            x = s_from.x + int(((s_to.x - s_from.x) / s_from.euclidean_distance(s_to))*d)
            y = s_from.y + int(((s_to.y - s_from.y) / s_from.euclidean_distance(s_to))*d)
            line = State(x, y, None)
            if not self.state_is_free(line):
                return False
    ...
    def plan(self, start_state, dest_state, max_num_steps, max_steering_radius, dest_reached_radius, live_view=True):
        ...
        for step in range(max_num_steps):
            ...
            # in the output.
            s_in = self.sample_state()
            s_nearest = self.find_closest_state(tree_nodes, s_in)
            s_new = self.steer_towards(s_nearest, s_in, max_steering_radius)
